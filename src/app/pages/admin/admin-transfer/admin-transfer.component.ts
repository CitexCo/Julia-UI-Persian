...
// admin transfer components shows the list of approved token transfer list
@Component({
...
export class AdminTransferComponent implements OnInit {
  ...
  constructor(
    ...
    this.adminService.alltransferList().subscribe(data => {
      ...
      this.pendingTransferDataSource = new MatTableDataSource(
  ...
  // send a transfer transaction to the contract
  async transfer(element) {
    try {
      console.log(this.account.address);

      let transaction = await this.tokenService.transferToken(
  ...
  // gets account information and render account info in pop-up
  async showAccount() {
    try {
      this.account = await this.tokenService.getAccountInfo();
  ...
  // gets token information at a specified address and render token info in pop-up
  async showContract() {
    try {
      this.token = await this.tokenService.getTokenInfo();
  ...
  // opens a snack and show a message
...
const INIT_DATA: TokenTransfer[] = [
  {
    ...
    to: "0xc3c49e620b250e1421f9054eac5b230f9730b048",
    from: "0xb3eece8b0d3a389fd182e46ef55b3a6e270cfb17",
    amount: "2"
  },
  {
    receiptNumber: 2,
...
export class AccountInfoComponent implements OnInit {
  ...
  // shows account info a new inserted account
...
export class TokenInfoComponent implements OnInit {
  ...
  // shows the token info a new inserted token address
