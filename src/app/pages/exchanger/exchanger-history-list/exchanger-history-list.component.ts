...
export class ExchangerHistoryListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.exchangerService.getList().subscribe(receipts => {
      this.receipts = this.initializeReceiptItems(receipts);
      this.dataSource.data = this.receipts;
    });
  }

  // filters receipt on checkbox click, shows or removes poending list
  filterReceipts(e) {
    if (e.checked) {
      this.exchangerService.getPendingList().subscribe(receipts => {
        this.receipts = this.initializeReceiptItems(receipts);
        this.dataSource.data = this.receipts;
      });
    } else {
  ...
  // maps a set of receibed receipts into a Receipt model form
  initializeReceiptItems(receipts): UserReceipt[] {
    let receipt = receipts.map((receipt: UserReceipt) => {
      return receipt;
    });
    return receipt;
  }

  // applies filter to show or remove pending list
