...
export class ExchangerUserListComponent implements OnInit {
  ...
  // opens a snack bar and shows a message
...
// exchanger verifiv\cation component
@Component({
...
export class ExchangeVerificationComponent {
  ...
  // toggles between personal and passport image on click event
  toggleImageSource() {
    //console.log('toggle')
    if (this.showPersonelImage) {
      ...
      this.imageSource = this.imageService.getImg(
        this.userInfo.passportImageAddress
      );
    }
  }

  // verifies image for extension and image size
  ...
  // selects an image from local address
  selectImage(event) {
  ...
  // removes the selected image to choose a new one
  removeImage() {
    this.photo = null;
  }

  // confirms the receipt on button click
  confirmUser(comment) {
    this.querySent = true;
    let receipt = {
