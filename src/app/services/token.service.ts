...
export class TokenService {
  ...
  // returns the CitexToken contract at its address
  ...
  // returns general token information (name, su=ymbol, decimals, totalSupply, address)
  ...
  async getTokenInfo(_address?: string): Promise<Token> {
    let CitexToken = _address
      ? await this._citexToken(_address)
      : await this._citexToken();
  ...
  // returns the current active account address
  async getAccount() {
  ...
  // get account info of an address (address, ether balance, token balance)
  ...
  // transfers amount of token to a specified address
  ...
  // sets an email hash  with a related balance
